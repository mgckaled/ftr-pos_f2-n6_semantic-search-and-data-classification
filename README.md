...
Este projeto implementa e compara três abordagens complementares para classificação multimodal de imagens, utilizando um dataset balanceado de mil fotografias de felinos e caninos para avaliar empiricamente trade-offs entre precisão, latência e consumo de recursos computacionais. A primeira abordagem emprega embeddings multimodais CLIP (Contrastive Language-Image Pre-training) combinados com classificador K-Nearest Neighbors baseado em similaridade de cossenos, oferecendo implementação trivial sem necessidade de treinamento e alcançando acurácia de 99% em conjunto de teste de duzentas imagens. A segunda abordagem explora capacidades emergentes de Large Language Models através de classificação zero-shot com Gemini 2.0 Flash, demonstrando performance equivalente (99% de acurácia) mediante prompting estruturado, porém impondo restrições de rate limiting (15 requisições por minuto) e latência significativamente superior. A terceira abordagem propõe um sistema ensemble que combina ambos métodos através de voting condicional: utiliza predições do classificador KNN quando a confiança probabilística excede 80%, delegando casos incertos ao LLM para mitigar erros em regiões de baixa separabilidade no espaço de embeddings. Análise exploratória mediante redução dimensional T-SNE revelou excelente separabilidade entre classes (Silhouette Score: 0.708), corroborando a efetividade de representações vetoriais CLIP para capturar características visuais discriminativas entre taxonomias animais, enquanto visualizações de matrizes de confusão e análise de exemplos erroneamente classificados demonstram que ambas abordagens apresentam complementaridade em termos de precision-recall trade-offs por classe.

- [Projeto: Classificação de Imagens](./src/project1/) - Notebook Jupyter completo com implementação das três abordagens, visualizações T-SNE, métricas detalhadas e serviço unificado de classificação.

---

### Sistema de Recomendação de Filmes

Este projeto implementa e otimiza empiricamente três abordagens fundamentais para sistemas de recomendação, utilizando o dataset MovieLens 990k (HuggingFace) contendo 990.425 ratings de 43.584 usuários sobre 15.276 filmes, caracterizado por sparsidade extrema de 99.86% que impõe desafios significativos para algoritmos de collaborative filtering. A primeira abordagem emprega filtragem colaborativa item-item através de K-Nearest Neighbors baseado em similaridade de cossenos entre vetores de ratings, implementando otimizações de memória mediante estruturas esparsas (scipy.sparse.csr_matrix) e armazenamento top-K de similares (K=2000) que reduzem consumo de RAM de 2.5 GB para 230 MB, alcançando Precision@10 de 7.8% e Recall@10 de 21% após tuning de hiperparâmetros. A segunda abordagem explora filtragem baseada em conteúdo utilizando embeddings semânticos de 384 dimensões gerados por Sentence-Transformers (all-MiniLM-L12-v2) a partir de título e gêneros dos filmes, construindo perfis de usuários como médias ponderadas de embeddings de itens avaliados e computando recomendações via similaridade de cossenos, resultando em performance inferior (Precision@10: 0.7%) devido à limitação de metadados disponíveis. A terceira abordagem propõe sistema híbrido adaptativo que combina ambos métodos através de ponderação dinâmica α calculada em função do número de ratings do usuário (α = min(0.9, 0.3 + 0.6 × num_ratings/50)), favorecendo collaborative filtering para usuários experientes e content-based para cenários de cold-start, atingindo Precision@10 de 7.1% com NDCG@10 de 0.583. Análise comparativa de configurações revelou que threshold de relevância MIN_RATING_THRESHOLD=3.0 e TOP_K_SIMILAR=2000 constituem ponto ótimo de trade-off entre precision (+34% vs configuração conservadora) e tempo de processamento (~4 minutos), enquanto redução dimensional T-SNE dos embeddings demonstra agrupamentos semânticos coerentes por gênero cinematográfico, validando a capacidade de modelos transformer em capturar similaridade temática a partir de metadados textuais limitados. Sistema interativo implementado com Jupyter Widgets permite seleção de 10 filmes com ratings 1-5, geração de recomendações personalizadas via três métodos comparáveis em tabs, exploração de filmes similares por conteúdo, e análise visual de perfil do usuário incluindo distribuição de gêneros preferidos e recomendação automática de α ideal baseada em estatísticas de avaliações fornecidas.

- [Projeto: Sistema de Recomendação](./src/project2/) - Notebook Jupyter com implementação otimizada das três abordagens, métricas comparativas (Precision/Recall/NDCG@K), visualizações T-SNE, análise cold-start e interface interativa completa.
